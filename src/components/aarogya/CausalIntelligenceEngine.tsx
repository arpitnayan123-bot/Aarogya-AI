...
/**
 * CausalIntelligenceEngine — The "Why & Root Cause" module.
 *
 * 5 panels:
 * 1. Primary Insight Card — clear root cause explanation
 * 2. Causal Chain Visualization — interactive directed graph
 * 3. Contribution Analysis — percentage impact per factor
 * 4. What-If Simulator — modify variables, see predicted outcomes
 * 5. Confidence & Uncertainty — confidence score + alternatives
 *
 * Powered by the causalEngine.ts reasoning core.
 */

import React, { useState, useMemo } from 'react';
import {
  Brain, GitBranch, Target, Zap, AlertTriangle, CheckCircle2,
  TrendingUp, TrendingDown, Activity, Sparkles, ChevronRight,
  Plus, Minus, Clock, ShieldQuestion, Network
} from 'lucide-react';
import {
  analyzeRootCauses, simulateIntervention, getAbnormalNodes, getNodeById,
  type CausalAnalysis, type InterventionResult
} from '@/lib/causalEngine';

const ABNORMAL_TARGETS = getAbnormalNodes().filter(n => n.type === 'observed');

export const CausalIntelligenceEngine: React.FC = () => {
  ...
  const runSimulation = () => {
    ...
    getAbnormalNodes().forEach(n => { if (n.currentValue !== undefined) nodeValues[n.id] = n.currentValue; });
    const result = simulateIntervention(simNodeId, simChange, nodeValues);
    setSimResult(result);
  };

  const target = getNodeById(selectedTarget);
    ...
    <div className="space-y-6 animate-fadeIn">
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold flex items-center gap-2">
              Health Causality
              <span className="text-[9px] font-bold bg-amber-400 text-amber-900 px-2 py-0.5 rounded-full">CIE</span>
            </h1>
        ...
        <div className="flex flex-wrap gap-2">
          {ABNORMAL_TARGETS.map(node => (
            ...
            >
              ...
              {node.label}
              <span className="text-[10px] text-slate-400">{node.currentValue}{node.unit}</span>
            </button>
          ))}
        </div>
      </div>

      {/* Panel 1: Primary Insight Card */}
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">AI-generated causal explanation</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 mt-4">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-violet-500/20 border border-violet-500/30">
              ...
              <span className="text-xs font-bold text-violet-300">{analysis.confidence}% confidence</span>
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20">
              <AlertTriangle className="w-3.5 h-3.5 text-amber-400" />
              <span className="text-xs font-bold text-amber-300">{analysis.rootCauses.length} root causes found</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm">
        <h2 className="font-extrabold text-slate-900 mb-4 flex items-center gap-2">
          <GitBranch className="w-5 h-5 text-violet-600" /> Causal Chain Visualization
        </h2>
        {analysis.rootCauses.length > 0 ? (
                        ...
                        return (
                          <div key={j} className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] text-slate-400 flex items-center gap-0.5">
                              <Clock className="w-2 h-2" /> {edge.timeDelay === 'immediate' ? 'hrs' : edge.timeDelay === 'medium' ? 'days' : 'wks'}
                            </span>
                          </div>
                        );
                      })}
                ...
                {cause.modifiable && (
                  <div className="mt-2 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-[10px] font-bold text-emerald-700 uppercase">Modifiable — Fix This</span>
                    </div>
          ...
          <p className="text-sm text-slate-400 text-center py-4">No causal paths found for this variable.</p>
        )}
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm">
        <h2 className="font-extrabold text-slate-900 mb-4 flex items-center gap-2">
          <Target className="w-5 h-5 text-violet-600" /> Contribution Analysis
        </h2>
          ...
          {analysis.rootCauses.map((cause, i) => (
            <div key={i}>
              ...
              <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-1000 ${cause.modifiable ? 'bg-gradient-to-r from-emerald-500 to-teal-500' : 'bg-gradient-to-r from-slate-400 to-slate-500'}`}
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <span className="text-[10px] text-slate-400 flex items-center gap-0.5">
                  <Clock className="w-2.5 h-2.5" /> {cause.timeToEffect === 'immediate' ? 'Hours' : cause.timeToEffect === 'medium' ? 'Days' : 'Weeks/Months'}
                </span>
              </div>
            </div>
          ))}
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm">
        <h2 className="font-extrabold text-slate-900 mb-4 flex items-center gap-2">
          <Zap className="w-5 h-5 text-amber-500" /> What-If Simulator
          <span className="text-xs font-normal text-slate-400 ml-1">do(variable = change) → predict outcomes</span>
        </h2>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1 block">Variable to change:</label>
            <select value={simNodeId} onChange={e => setSimNodeId(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500">
              {['sleep_hours', 'salt_intake', 'steps', 'sugar_intake', 'bmi'].map(id => {
                const n = getNodeById(id);
                return <option key={id} value={id}>{n?.label} ({n?.currentValue}{n?.unit})</option>;
              })}
            </select>
          </div>
          <div>
            <label className="text-xs font-bold text-slate-500 mb-1 block">Change amount: {simChange > 0 ? '+' : ''}{simChange} {simNode?.unit}</label>
            <div className="flex items-center gap-2">
        ...
        <button onClick={runSimulation}
          className="w-full bg-gradient-to-r from-violet-600 to-purple-600 text-white font-bold py-3 rounded-2xl flex items-center justify-center gap-2 hover:scale-[1.01] transition-transform shadow-lg shadow-violet-600/20">
          ...
          <div className="mt-4 space-y-3 animate-fadeIn">
            <div className="p-3 bg-violet-50 rounded-xl border border-violet-100">
              ...
              <div className="flex items-center gap-1.5 mt-2">
                <ShieldQuestion className="w-3 h-3 text-violet-500" />
                <span className="text-[10px] font-bold text-violet-600">{simResult.confidence}% confidence in prediction</span>
              </div>
            </div>
              ...
              {simResult.affectedVariables.slice(0, 6).map((v, i) => (
                <div key={i} className="flex items-center gap-3 p-2.5 bg-slate-50 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-sm text-slate-900">{v.nodeLabel}</div>
                    <div className="text-xs text-slate-500">{v.oldValue} → {v.predictedValue}</div>
                  </div>
                  <div className="text-right">
                    <div className={`text-sm font-extrabold ${v.changePercent > 0 ? 'text-red-500' : v.changePercent < 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
                      {v.changePercent > 0 ? '+' : ''}{v.changePercent}%
                    </div>
                    <div className="text-[8px] text-slate-400 flex items-center gap-0.5 justify-end">
                      <Clock className="w-2 h-2" /> {v.timeToEffect === 'immediate' ? 'hrs' : v.timeToEffect === 'medium' ? 'days' : 'wks'}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm">
        <h2 className="font-extrabold text-slate-900 mb-4 flex items-center gap-2">
          <ShieldQuestion className="w-5 h-5 text-violet-600" /> Confidence & Uncertainty
        </h2>
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="relative w-20 h-20 flex-shrink-0">
            <svg className="w-full h-full -rotate-90" viewBox="0 0 80 80">
              ...
              <circle cx="40" cy="40" r="34" fill="none" stroke={analysis.confidence > 75 ? '#10b981' : analysis.confidence > 50 ? '#f59e0b' : '#ef4444'} strokeWidth="6" strokeLinecap="round"
                strokeDasharray={`${2 * Math.PI * 34}`} strokeDashoffset={`${2 * Math.PI * 34 * (1 - analysis.confidence / 100)}`} />
            </svg>
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-lg font-extrabold text-slate-900">{analysis.confidence}%</span>
            </div>
          </div>
          <div>
            <div className="font-bold text-sm text-slate-900">Overall Confidence</div>
            <p className="text-xs text-slate-500">
              {analysis.confidence > 75 ? 'High confidence — multiple strong causal paths identified' : analysis.confidence > 50 ? 'Moderate confidence — some causal evidence available' : 'Low confidence — limited data for causal inference'}
            </p>
          </div>
        </div>
        ...
        {analysis.latentInferences.length > 0 && (
          <div className="mb-4">
            <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1">
              <Brain className="w-3 h-3" /> Latent State Inferences
            </div>
            <div className="space-y-2">
              {analysis.latentInferences.map((inf, i) => (
                <div key={i} className="p-3 bg-violet-50 rounded-xl border border-violet-100">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-sm text-slate-900">{inf.label}</span>
                    <span className="text-xs font-extrabold text-violet-600">{inf.inferredValue.toFixed(1)}</span>
                  </div>
                  <div className="text-[10px] text-slate-500 mb-1">Evidence:</div>
                  <ul className="text-[11px] text-slate-600 space-y-0.5">
                    {inf.evidence.map((e, j) => <li key={j} className="flex items-start gap-1"><span className="text-violet-400 mt-0.5">•</span> {e}</li>)}
                  </ul>
        ...
        {analysis.alternativeHypotheses.length > 0 && (
          <div>
            <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1">
              <AlertTriangle className="w-3 h-3" /> Alternative Hypotheses
            </div>
            <div className="space-y-1.5">
              {analysis.alternativeHypotheses.map((hyp, i) => (
                <div key={i} className="flex items-start gap-2 p-2 bg-amber-50 rounded-lg">
                  ...
                  <span className="text-xs text-slate-700">{hyp}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-amber-700">
          Causal analysis is based on statistical causal models and medical literature. It represents probable causes, not definitive diagnoses.
          Always consult a healthcare professional for medical evaluation. Confidence scores reflect data quality and model certainty, not medical certainty.
        </p>
      </div>
    </div>
  );
...
export default CausalIntelligenceEngine;
